 ...
 * 
 * Features:
 * - Web server at (see serial monitor at 115200 for ip address) for firmware uploads
...
// Copy secrets.h.example to secrets.h and add your credentials
const char* ssid = WIFI_SSID;
const char* password = WIFI_PASSWORD;
// Using DHCP - static IP should be set on your router if needed
...
void setup() {
    ...
    // Initialize control pins BEFORE WiFi (no conflict)
    ...
    // Connect to WiFi BEFORE initializing Serial1 on GPIO43/44
    // (GPIO43/44 conflicts with USB Serial on ESP32-S3)
    ...
    while (WiFi.status() != WL_CONNECTED && attempts < 30) {
        delay(500);
        Serial.print(".");
        setLedColor(255, 0, 0);  // Red while connecting
        attempts++;
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.println("Check SSID and password in secrets.h");
    ...
    Serial.println("\n*** Initializing Teensy serial ports ***");
    Serial.println("*** USB Serial may become unstable after this point ***");
    Serial.println("*** Use web interface for monitoring ***\n");
    Serial.flush();  // Make sure all output is sent
    delay(100);
    
    // Initialize Teensy Flash Serial (GPIO43/44 - conflicts with USB Serial!)
    // This MUST be done AFTER all important Serial output
    TEENSY_FLASH_SERIAL.begin(115200, SERIAL_8N1, TEENSY_FLASH_RX_PIN, TEENSY_FLASH_TX_PIN);
    
    // Initialize Teensy Debug Serial (GPIO17/18 - no conflict)
