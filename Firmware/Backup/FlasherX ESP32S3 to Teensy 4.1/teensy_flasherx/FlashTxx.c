...
int firmware_buffer_init( uint32_t *buffer_addr, uint32_t *buffer_size )
{
  ...
  // attempt to malloc() RAM for buffer - if it fails, fall through to flash buffer
  *buffer_addr = (uint32_t)malloc( RAM_BUFFER_SIZE );
  ...
  // malloc failed, fall through to flash buffer
  #endif
  ...
  *buffer_size = FLASH_BASE_ADDR - *buffer_addr + FLASH_SIZE - FLASH_RESERVE;
  ...
  if (*buffer_size < 4096) {
    return( NO_BUFFER_TYPE );  // Not enough space
  }
