...
void FlasherXOTA::handleCommand(uint8_t cmd)
{
    switch (cmd) {
        case CMD_PING:
            ...
            if (_usbSerial) _usbSerial->println("[OTA] Ping received");
            if (_debugSerial) _debugSerial->println("Ping OK");
            break;
        ...
        case CMD_VERSION:
            ...
            if (_usbSerial) _usbSerial->println("[OTA] Version requested");
            if (_debugSerial) _debugSerial->println("Version sent");
            break;
